...
resource "aws_security_group" "diamond_dogs_admin" {
...
resource "aws_instance" "diamond_dogs" {
  ...
  subnet_id                   = aws_subnet.diamond_dogs.id
  vpc_security_group_ids      = [aws_security_group.diamond_dogs.id, aws_security_group.diamond_dogs_admin.id]
  key_name                    = aws_key_pair.vex_public_key.id
